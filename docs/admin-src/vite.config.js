        ...
        const graph = [
          {
            ...
            name: 'Protokol Kartları Admin Paneli',
            applicationCategory: 'DeveloperApplication',
            operatingSystem: 'Any',
            softwareVersion: pkg.version,
            url: SITE,
            description: pkg.description,
            author: { '@type': 'Person', name: 'Arda Bilasa' }
          }
        ];
  ...
  return {
    ...
    transformIndexHtml: {
      ...
      handler(html) {
        ...
        // subpath (e.g. /admin/) resolve correctly.
...
<link rel="apple-touch-icon" href="${base}icon-192.png">`;
        ...
        // description (page-specific copy wins). Title falls back to "Protokol"
        ...
        if (!/name=["']description["']/i.test(out)) {
          ...
          const title = titleMatch ? titleMatch[1].replace(/\s+\|\s+.*$/, '').trim() : 'Protokol';
          const bcMatch = /data-breadcrumb=["']([^"']+)["']/i.exec(out);
          // Strip any "|href" targets — the description wants labels only.
          const breadcrumb = bcMatch
            ? bcMatch[1].replace(/\|[^>]*/g, '').replace(/^(Home|Anasayfa)\s*>\s*/, '').replace(/\s*>\s*/g, ' > ').trim()
            : '';
          const desc = breadcrumb
            ? `${title} — ${breadcrumb}. OMÜ Protokol Kartları admin paneli.`
            : 'OMÜ Protokol Kartları admin paneli.';
