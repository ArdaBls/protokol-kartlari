// OMÜ birim koordinatları -- eski admin-src/src/v4/unit-coords.js'nin birebir karşılığı.
// Anahtarlar FACULTY_GROUPS listesindeki isimlerle eşleşir (protocolRules.ts). Koordinatı
// olmayan birimler burada yok -- harita bunları göstermez, sağ panelde "konumu bilinmeyen" notuyla belirir.
export const UNIT_COORDS: Record<string, [number, number]> = {
...
// "Düzenleyen Birim" alanı FACULTY_GROUPS'tan seçildiği için gerçek kayıtlarda "Rektörlük"
// değil "Rektör" / "Rektör Yardımcısı" görülür -- ikisi de aynı fiziksel binaya işaret eder.
const ALIASES: Record<string, string> = { Rektör: 'Rektörlük', 'Rektör Yardımcısı': 'Rektörlük' }

export function resolveUnitName(birim?: string): string {
  const b = String(birim ?? '').trim()
  return ALIASES[b] || b
}

export function coordsFor(birim?: string): [number, number] | null {
